...
class StaticDisco:
    ...
    def _add_node(self, jid: OptJidStr = None, node: Optional[str] = None,
                  ifrom: OptJidStr = None) -> NodeType:
        """
        Create a new set of stanzas for the provided
        JID and node combination.

        :param jid:  The JID that will own the new stanzas.
        :param node: The node that will own the new stanzas.
        :param ifrom: The JID that sent the new stanzas.
        """
        if jid is None:
    ...
    def _get_node(self, jid: OptJidStr = None, node: Optional[str] = None,
                  ifrom: OptJidStr = None) -> NodeType:
        if jid is None:
            node_jid = self.xmpp.boundjid.full
        elif isinstance(jid, JID):
        ...
        if ifrom is None:
            ...
            node_ifrom = ifrom
        if (node_jid, node, node_ifrom) not in self.nodes:
            self._add_node(node_jid, node, node_ifrom)
        return self.nodes[(node_jid, node, node_ifrom)]

    def _node_exists(self, jid: OptJidStr = None, node: Optional[str] = None,
    ...
    def get_info(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        """
        if not self._node_exists(jid, node):
            if not node:
                return DiscoInfo()
            else:
                raise XMPPError(condition='item-not-found')
        else:
            return self._get_node(jid, node)['info']
    ...
    def set_info(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        """
        new_node = self._add_node(jid, node)
        new_node['info'] = data
    ...
    def del_info(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        The data parameter is not used.
        """
        if self._node_exists(jid, node):
            self._get_node(jid, node)['info'] = DiscoInfo()
    ...
    def get_items(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        if not self._node_exists(jid, node):
            ...
            return self._get_node(jid, node)['items']
    ...
    def del_items(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        """
        if self._node_exists(jid, node):
            self._get_node(jid, node)['items'] = DiscoItems()
    ...
    def del_identity(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        """
        if self._node_exists(jid, node):
            self._get_node(jid, node)['info'].del_identity(
                    data.get('category', ''),
                    data.get('itype', ''),
                    data.get('name', None),
                    data.get('lang', None))
    ...
    def del_identities(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        """
        if self._node_exists(jid, node):
            del self._get_node(jid, node)['info']['identities']
    ...
    def del_feature(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        """
        if self._node_exists(jid, node):
            self._get_node(jid, node)['info'].del_feature(
                    data.get('feature', ''))
    ...
    def del_features(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        if not self._node_exists(jid, node):
            return
        del self._get_node(jid, node)['info']['features']
    ...
    def del_item(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        """
        if self._node_exists(jid, node):
            self._get_node(jid, node)['items'].del_item(
                    data.get('ijid', ''),
    ...
    def cache_info(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        new_node = self._add_node(jid, node, ifrom)
        new_node['info'] = info
    ...
    def get_cached_info(self, jid: OptJid, node: Optional[str], ifrom: OptJid,
        ...
        The data parameter is not used.
        """
        if not self._node_exists(jid, node, ifrom):
            return None
        return self._get_node(jid, node, ifrom)['info']
