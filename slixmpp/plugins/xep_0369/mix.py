...
class XEP_0369(BasePlugin):
    ...
    async def join_channel(self, room: JID, nick: str, subscribe: Set[str]) -> Set[str]:
        """
        Join a MIX channel.
    ...
    async def update_subscription(self, room: JID, subscribe: Optional[Set[str]] = None,
                                  unsubscribe: Optional[Set[str]] = None) -> Tuple[bool, Tuple[Set[str], Set[str]]]:
        """
    ...
    async def leave_channel(self, room: JID) -> Iq:
        """"
        Leave a MIX channel
    ...
    async def set_nick(self, room: JID, nick: str) -> str:
        """
    ...
    async def create_channel(self, service: JID, channel: Optional[str] = None) -> str:
        """
        ...
        iq = self.xmpp.make_iq_set(ito=service.server)
    ...
    async def destroy_channel(self, service: JID, channel: str) -> Iq:
        """
        ...
        """
        iq = self.xmpp.make_iq_set(ito=service)
        iq['mix_destroy'] = channel
        return await iq.send()

    async def list_mix_nodes(self, channel: JID) -> Set[str]:
        """
        ...
        """
        result = await self.xmpp['xep_0030'].get_items(channel, node='mix')
        nodes = set()
        for item in result['disco_items']:
            nodes.add(item['node'])
        return nodes

    async def list_participants(self, channel: JID) -> List[Tuple[str, str, JID]]:
        """
        ...
        """
        info = await self.xmpp['xep_0060'].get_items(channel, 'urn:xmpp:mix:nodes:participants')
        participants = list()
        for item in info['pubsub']['items']:
            identifier = item['id']
    ...
    async def list_channels(self, service: JID) -> List[Tuple[JID, str]]:
        """
        ...
        """
        results_stanza = await self.xmpp['xep_0030'].get_items(service.server)
        results = []
